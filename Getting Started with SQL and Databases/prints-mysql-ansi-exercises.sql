/**	MySQL / MariaDB Quirks
	================================================
	Only ISO8601 Date Literals are accepted.
	You can use:
		SELECT * FROM customers
		WHERE dob=str_to_date('09-NOV-1989', '%d-%M-%Y');

	SELECT *,column		--	ok
	SELECT column,*		--	fails

	Cannot OFFSET by Calculated Value

	SET SESSION sql_mode = 'ANSI';

	Beware Traditional Mode:
...
/*	3.16: Single and Double Quotes
	...
	SELECT * FROM customers WHERE givenname="Town";
...
/*	3.57: Regular Expressions
	...
	SELECT *
	FROM customers
	WHERE lower(familyname) REGEXP '^[a-k][^hy]';
...
/*	4.09: Sorting Dates
	...
	SELECT * FROM sorting ORDER BY str_to_date(datestring,'%d %M %Y');
...
/*	4.22: Paged Results
	...
	--	Page 4 (skip 3 pages)
		SELECT * FROM paintings
		ORDER BY title LIMIT 20 OFFSET 60;
	--	Reverse Order: Last page first
		SELECT * FROM paintings
		ORDER BY title DESC LIMIT 20 OFFSET 0;
...
/*	4.25: Arbitrary String Sort
	...
	SELECT *
	FROM sorting
	ORDER BY INSTR('One,Two,Three,Four,Five,Six,Seven,Eight,Nine',numbername);
...
/*	5.09: Coalesce Missing String
	...
	--	In Traditional Mode
		...
		SELECT
			id, givenname, familyname,
			concat(coalesce(concat(givenname,' '),''),familyname) AS fullname
		FROM artists;
...
/*	5.21: Number Formatting
	...
	--	Current Locale
		SELECT concat('$',format(total,2))
		FROM sales;
	--	Manual Locale
		SELECT concat('€',format(total,2,'de_DE'))
		FROM sales;

/*	5.22: Current Date & Time
	================================================ */

	SELECT current_timestamp, now();	--	aka now()
...
/*	5.34: Substring
	================================================
	aka	substr(string,start,length)
	================================================ */

	SELECT substring('abcdefghijklmnop',3,5);	--	Results in: cdefg

/*	5.35: Left & Right
	================================================ */

	SELECT left('abcdefghijklmnop',5);
	SELECT substr('abcdefghijklmnop',1,5);
...
/*	5.36: Format Phone Number
	...
	SELECT
		id, givenname, familyname,
		concat(left(phone,2),' ',substr(phone,3,4),' ',right(phone,4)) AS phone
	FROM customers;
...
/*	5.42: Cast to String
	...
	--	Mixed Types
		...
		SELECT id||': '||givenname||' '||familyname||' - '||dob AS info
		FROM customers;
...
/*	5.44: Cast to Date
	...
	--	This works
		SELECT cast('1969-07-20' as date) AS moon_landing;
		SELECT str_to_date('20 Jul 1969','%d %M %Y') AS moon_landing;
...
/*	6.22: Format as Birthday
	...
	SELECT date_format(dob,'%m-%d') AS birthday FROM customers;
...
/*	7.26: Redundant Groups
	...
	SELECT
		date_format(ordered,'%W') AS dayname,
		sum(total) AS total
	FROM sales
...
/*	7.31: Summarising Strings
	================================================
	Standard: Use a.givenname||' '||a.familyname
	Oracle: No AS
	...
	--	MySQL / MariaDB:
		SELECT id, familyname, givenname, group_concat(artist SEPARATOR ', ')
	--	SQLite:
		SELECT id, givenname, familyname, group_concat(artist, ', ')
	================================================ */
	...
	SELECT id, givenname, familyname, group_concat(artist SEPARATOR ', ')
	FROM cte
